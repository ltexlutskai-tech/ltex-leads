...
console.log("\n=== Повторний запит: просто через пробіл (без роздільника) ===");
f = setup();
f.main.getRange(5, 12).setValue("Готово");
post("/передати 0671234567 Дунас Богдан Питає ціну на палету взуття, писав у TikTok", ADMIN, "Адмін");
ok(f.main.getRange(5, 11).getValue() === "Дунас Богдан", "імʼя менеджера розпізнано серед тексту");
ok(/Питає ціну на палету взуття, писав у TikTok/.test(viber("VID_DUN")), "решта тексту стала описом запиту");
ok(!/Питає/.test(String(f.main.getRange(5, 11).getValue())), "опис не потрапив у поле менеджера");
ok(mainRow(f)[11] === "Очікує", "статус повернувся в «Очікує»");

console.log("\n=== Повторний запит: скорочене імʼя + опис через пробіл ===");
f = setup();
post("/передати 0671234567 Дунас питав про доставку і терміни", ADMIN, "Адмін");
ok(f.main.getRange(5, 11).getValue() === "Дунас Богдан", "скорочене імʼя розпізнано");
ok(/питав про доставку і терміни/.test(viber("VID_DUN")), "опис після скороченого імені");

console.log("\n=== Слово з опису збігається з прізвищем іншого менеджера ===");
f = setup();
post("/передати 0671234567 Дунас Богдан Гуменюк передав клієнта, просить каталог", ADMIN, "Адмін");
ok(f.main.getRange(5, 11).getValue() === "Дунас Богдан", "менеджером лишився той, чиє імʼя стоїть першим");
ok(/Гуменюк передав клієнта, просить каталог/.test(viber("VID_DUN")), "решта тексту — опис");

...
Telegram повідомляє не лише про прихід, а й про відхід — і ми на це підписані:

| Що сталось | Статус у рядку | Що бачить менеджер |
|---|---|---|
| Відписався від каналу (або його звідти прибрали) | `🚪 Відписався` | Viber-сповіщення одразу + рядок у треді |
| Заблокував бота | `⛔ Заблокував бота` | те саме, з попередженням, що писати в Telegram більше не можна |
| Розблокував | `💬 У боті` | — |
| Підписався знову | `👤 Приєднався` | — |

**Дата приєднання при відписці НЕ стирається.** Вона лишається історією: за
нею видно, скільки людина протрималась. Статус показує теперішнє, дата —
минуле.

**Про підписку у Viber не пишемо, про відписку — пишемо.** Підписка й так
видно у звіті, а зайвий шум притуплює увагу до того, що справді вимагає дії.
Втрата вимагає: саме після неї ще є про що поговорити з клієнтом.

У щоденному звіті поруч із «Приєдналось» тепер стоять «Відписались» і
«Заблокували бота» — інакше зростання виглядає більшим, ніж воно є.

### Що на сторінці кнопки

Картка клієнта, **готове повідомлення** з кнопками месенджерів і кнопка
історії. Окремої картки з посиланням немає: воно останнім рядком у самому
повідомленні — так месенджер малює картку каналу, і саме повідомлення
менеджер і надсилає. Два місця з тим самим посиланням давали лише зайвий
екран і шанс скопіювати не те.

Картка з посиланням показується **тільки** тоді, коли посилання немає, — з
поясненням, чого бракує.

### Ручні статуси менеджера

У списку статусів є два, які ставить людина, а не система:

| Статус | Коли |
|---|---|
| `🚫 Не потрібно` | Клієнт не користується Telegram або канал йому не потрібен |
| `❌ Відмовився` | Сказав прямо, що не хоче |

**Бот їх ніколи не перезаписує** — рішення менеджера головніше за автоматику.
І такий рядок **виходить з обліку** в щоденному звіті: він не «надісланий»,
але й не «залишився». Раніше висів у «залишилось надіслати» вічним боргом,
який неможливо закрити: менеджер уже вирішив, що клієнту канал не потрібен, а
звіт щоранку вимагав від нього надіслати. У звіті такі рядки тепер рахуються
окремим рядком «Не потрібно / відмовились».

Передумали — **очистіть клітинку**: рядок повернеться у звичайну роботу.

### Коли саме зʼявляється «✅ Надіслано»

**Відкриття сторінки статусу НЕ ставить.** Його ставить дія менеджера на
сторінці: «Скопіювати посилання», «Скопіювати повідомлення», перехід у Viber /
Telegram / WhatsApp — або кнопка **«✅ Позначити надісланим»**, якщо надіслали
іншим шляхом (продиктували по телефону, переслали з іншого пристрою).

Чому так. Кнопка в таблиці — звичайне посилання, а за посиланнями ходять не
лише люди: сканери, антивірус у пошті, бот попереднього перегляду в месенджері,
префетч браузера. 14.09.2026 через це сім рядків отримали «✅ Надіслано» о
06:35 в одну хвилину, ще два — о 03:59 ночі. У примітці кожного стояло
«Джерело: кнопка в таблиці», хоч менеджер їх не чіпав. У звіті це виглядало як
зроблена робота, а клієнтам ніхто нічого не надсилав.

Правило просте: **запит на адресу нічого не міняє**. Міняє тільки дія на
сторінці — її робот не відтворить, бо для цього треба виконати скрипт сторінки
в сесії Google і натиснути кнопку.

Те саме стосується «Скасувати статус»: тепер це дія, а не перехід за адресою.

Персональні одноразові запрошення перегляд теж більше не створює — раніше
кожне відкриття адреси просило Telegram зробити нове.

#### Прибрати хибні позначки

Вони впізнаються за однаковим часом до хвилини. Спершу подивитись:

```
tgWhoMarked("14.09.2026 06:35")
```

Потім прибрати (перший виклик лише показує, нічого не міняючи):
...
Чіпає **лише** `✅ Надіслано`. Рядки, де стоїть `💬 У боті`, `👤 Приєднався`,
`🚪 Відписався` чи `⛔ Заблокував бота`, не чіпає ніколи: туди клієнт потрапив
сам, і хибними такі статуси бути не можуть. Посилання в рядку лишається — як і
при звичайному скасуванні.

### Історія роботи з клієнтом

Досі кожна система памʼятала своє й нічого — цілком. Таблиця тримала
**останній** статус у клітинці: змінився — попереднього більше немає. Наша
система памʼятала заявку, дзвінки й замовлення. Бот — тред переписки. На
питання «а що взагалі було з цим клієнтом?» відповіді не було ніде, і
менеджер, який брав чужого клієнта, починав з чистого аркуша.

Тепер журнал **один**, і він у нашій системі — там для нього вже є стрічка в
картці ліда й картці клієнта, з авторами, фільтрами й перепискою. Таблиця в
нього пише і з нього читає. Другий журнал у таблиці не заводимо свідомо: два
журнали завжди розходяться, і за півроку ніхто не скаже, який правильний.

Що тепер потрапляє в історію:

| Подія | Звідки |
|---|---|
| Натиск «📨 Надіслати» (вперше / повторно / скасування) | таблиця |
| Ручна зміна статусу запрошення, менеджера, статусу обдзвону, коментаря | таблиця |
| Клієнт відкрив бота за посиланням з рядка | бот |
| Підписався на канал · відписався · заблокував · розблокував бота | бот |
| Заявка, зміна етапу, дзвінки, замовлення, оплати, переписка | система |

Відкрити: кнопка **«📜 Історія роботи з клієнтом»** на сторінці, яка
зʼявляється після «📨 Надіслати». Саме там вона й потрібна — у момент, коли
менеджер вирішує, що людині написати. Окремої колонки в таблиці не додаємо:
вона коштувала б кнопки на кожному рядку.

Увімкнути (один раз):

```
setupTgHistoryTrigger()    // щоб ручні зміни в таблиці теж записувались
tgHistCheck()              // перевірка
```

Адресу системи шукати не треба: береться `TG_ECO_API`, а якщо її немає —
`LTEX_CRM_URL` (місток лідів уже вказує на той самий домен). Тримати одну
адресу у двох властивостях сенсу немає: розійдуться вони рівно тоді, коли
домен зміниться, а про другу ніхто не згадає.

Ключ доступу — `TG_API_SECRET`, а якщо його немає — `TG_LINK_SECRET`. У
нашій системі це `LEADS_IMPORT_SECRET` (той самий, що у містка лідів). У
більшості налаштувань це одне й те саме значення, і заводити окремо нічого
не треба.

**Тригер ручних змін — свій, окремий від `onEdit` у `Code.gs`.** Apps Script
дозволяє кілька обробників однієї події, тож чужий перетирати не треба.
Своїх записів скрипт не бачить (edit-тригери на них не спрацьовують) — сюди
доходить рівно те, що змінила людина руками, а решту система вже знає від
бота й від містка лідів.

**Якщо система недоступна** — кнопка в таблиці працює як раніше, а сторінка
історії прямо каже, що показує лише події таблиці. Мовчазний неповний список
гірший за відсутній: менеджер вирішив би, що більше нічого й не було.

**Мережевий запит не стоїть на шляху кліку.** Між натиском «📨 Надіслати» і
сторінкою менеджер чекає, тому запис в історію відкладено на той самий
момент, що й синхронізацію файлу менеджера, — уже після показу сторінки.
...
| U | Статус TG-посилання | `✅ Надіслано` · `💬 У боті` · `👤 Приєднався` · `🚪 Відписався` · `⛔ Заблокував бота` · `🚫 Не потрібно` · `❌ Відмовився` |
...
Колір статусу: 🟩 надіслано · 🟨 у боті · 🟦 приєднався · 🟧 відписався · 🟥 заблокував бота ·
🟨 клієнт є, посилання ще не надсилали.
...
| `TG_ECO_API` | адреса нашої системи для історії; якщо немає — береться з `LTEX_CRM_URL` |
| `TG_API_SECRET` | ключ до `/api/leads/history`; якщо немає — береться `TG_LINK_SECRET` |
...
| `setupTgHistoryTrigger()` / `removeTgHistoryTrigger()` | записувати ручні зміни в таблиці в історію роботи з клієнтом |
| `tgHistCheck()` | чи працює історія: адреса, ключ, тригер, відповідь системи |
...
| `tgClearMarksAt("14.09.2026 06:35")` | прибрати хибні позначки за цю хвилину (без `, true` — лише показує) |

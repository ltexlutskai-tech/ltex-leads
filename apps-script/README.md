...
**Чому сторінка відкривається швидше, ніж раніше.** Найповільніше — відкрити
окремий файл менеджера. Тепер у головну таблицю статус пишеться до показу
сторінки, а запис у файл менеджера сторінка замовляє сама, вже показавшись
(`tgFinishClick`). Якщо вкладку закрити миттєво, це донесе плановий
`tgRefreshJob` протягом 15 хвилин.

**Кнопки месенджерів несуть текст.** WhatsApp відкриває чат саме з номером
клієнта і одразу з текстом. Viber і Telegram так не вміють: вони або
відкривають чат за номером, або несуть текст — тому кнопок дві групи.
Верхня («з готовим текстом») відкриває список чатів із уже набраним
повідомленням, нижня — чат конкретного клієнта без тексту.

...
```
tgResetProgress()             // спершу лише покаже, що буде прибрано
tgResetProgress("почати")     // прибрати
tgResetProgress("почати все") // прибрати разом із посиланнями й ніками
```

Без слова «почати» функція нічого не змінює — тільки рахує й показує. Це
навмисно: помилково запущена функція не має нічого стерти.
...
| `tgResetProgress()` · `tgResetProgress("почати")` | почати з чистого аркуша (без слова «почати» — лише показ) |

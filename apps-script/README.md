...
| `TG_CLIENT_LINK` | `bot` (типово) — посилання на бота з міткою · `invite` — пряме запрошення |
...
Вимикається властивістю `TG_CLIENT_LINK` = `invite` — тоді, як раніше,
видається пряме запрошення в канал. Значення `bot` (типове) — через бота.

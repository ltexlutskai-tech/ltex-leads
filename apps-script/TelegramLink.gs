...
//
// Привітання («Вітаю! Це {manager}, компанія L-TEX») тут навмисно НЕМАЄ
// (рішення user, 18.09.2026): кожного клієнта менеджер спершу продзвонює, і
// знайомство вже відбулось голосом. Написане слідом «Вітаю, це Тарас»
// виглядало б так, наче дзвінка не було. Плейсхолдер {manager} лишається
// робочим — він потрібен тим, хто задає власний шаблон через
// TG_MSG_TEMPLATE.
//
// Посилання стоїть ОСТАННІМ рядком, і після нього немає ні крапки, ні тексту.
// Причина практична: месенджери будують картку попереднього перегляду й
// підсвічують адресу надійніше, коли вона завершує повідомлення, — а в Viber
// довга адреса посеред тексту часто взагалі лишається звичайним текстом.
var TG_MSG_DEFAULT =
  ...
  "Якщо щось буде незрозуміло — пишіть, підкажемо. 🙌\n\n" +
...
//
// Слово має збігатися зі статусом у таблиці («👤 Приєднався») і з тим, що
// шукає звіт у лозі. Раніше тут стояло «Підписався на канал», і звіт, який
// рахує рядки з приміткою «приєднався…», не бачив жодного вступу через бота
// системи: у таблиці статус був, а в підсумку — нуль (18.09.2026).
function tgEcoActLabel_(act) {
  if (act === "join")    return "Приєднався до каналу";
...
function tgLandingBody_(r) {
  ...
  if (r.noLink) {
    ...
    // Окремої картки з посиланням немає: воно останнім рядком у самому
    // повідомленні (так месенджер малює картку каналу). Але КНОПКА «лише
    // посилання» потрібна (повернуто на запит user, 18.09.2026): після
    // дзвінка менеджер часто дописує клієнту своїми словами, і весь готовий
    // текст йому тоді зайвий — потрібна сама адреса.
    ...
    h.push('<button class="btn btn-g" style="margin-top:8px" ' +
           'onclick="cp(' + tgJs_(r.link) + ',this)">' +
           '🔗 Скопіювати лише посилання</button>');
    h.push('</div>');

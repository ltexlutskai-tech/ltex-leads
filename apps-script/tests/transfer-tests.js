...
console.log("\nДубль: клієнт уже в потрібного, але лишився і в старого");
test("прибираємо зайву копію, а не переносимо вдруге", () => {
  const env = makeEnv({
    files: { "Гуменюк Євген": ["A1"], "Захарчук Олександра": ["A1"] },
    main: [row("A1", "Рома", "Захарчук Олександра")],
  });
  env.sandbox.resyncManagerAssignments(false);
  assert.deepStrictEqual(env.files["Гуменюк Євген"], [], "зайва копія мала зникнути");
  assert.deepStrictEqual(env.files["Захарчук Олександра"], ["A1"], "робоча копія мала лишитись");
});

test("за дубль менеджерам НЕ шлемо «вам передано контрагента» вдруге", () => {
  const env = makeEnv({
    files: { "Гуменюк Євген": ["A1"], "Захарчук Олександра": ["A1"] },
    main: [row("A1", "Рома", "Захарчук Олександра")],
  });
  env.sandbox.resyncManagerAssignments(false);
  // Повторне сповіщення за той самий рух людина читає як НОВУ роботу.
  assert.strictEqual(env.viber.length, 0, "Viber мав мовчати: нічого не переїжджало");
  assert.strictEqual(env.owners.length, 0);
...
test("справжній перенос і дубль в одному запуску не плутаються", () => {
  ...
  env.sandbox.resyncManagerAssignments(false);
  assert.deepStrictEqual(env.files["Гуменюк Євген"], []);
  assert.deepStrictEqual(env.files["Захарчук Олександра"].sort(), ["A1", "A2"]);
  // Сповіщення — рівно одне, за єдиний справжній переїзд (A2).
  assert.strictEqual(env.owners.length, 1);
});

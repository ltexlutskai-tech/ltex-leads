// ============================================================
// L-TEX CRM | Google Apps Script v6.9
// Нове у v6.9: перепризначення менеджера з переносом контрагента
//   між таблицями менеджерів. Уся логіка — в окремому файлі
//   ManagerTransfer.gs. У ЦЬОМУ файлі змінено лише два місця:
//   1) команда «/передати» в doPost (позначено «v6.9»);
//   2) рядок про неї в тексті /допомога.
//
// v6.8: у відповіді бота на перевірку номера блок
//   «🏭 Знайдено в базі 1С» додатково показує «Статус» і
//   «Оп статус» клієнта (колонки H та I аркуша 1С).
//   Змінені місця: ONS_COL (+STATUS, +OP_STATUS),
//   check1CByPhone() (читає та повертає обидва статуси),
//   findOnsCol_() — нова, шукає колонку за заголовком,
//   і три місця виводу: автоперевірка номера, команда /1с,
//   попередження про дубль у parseAndSave.
//
// v6.7: місток «бот → L-TEX CRM» додатково передає, ХТО
//   зареєстрував ліда (поле addedBy) і поточний «Оновлений
//   статус» обдзвону (поле callStatus). У нашій системі це
//   видно в деталях ліда, а «НПТ» одразу ставить етап воронки.
//   Змінені місця: pushLeadToLtexCrm(row, newId, addedBy) —
//   в самому низу файлу — і два її виклики
//   (parseAndSave → registrar, saveEmailLead → "Gmail").
//
// v6.6: місток «бот → L-TEX CRM» — щойно збережений лід
//   автоматично дублюється в нашу систему (Клієнти → Ліди)
//   через функцію pushLeadToLtexCrm (в самому низу файлу).
//
// v6.5: колонки «Оновлений статус» та
...
// ── v6.8: STATUS (H) і OP_STATUS (I) — статуси клієнта в 1С.
// Це значення за замовчуванням: реальні колонки шукаються за
// заголовком у рядку 1 (див. findOnsCol_), тож зсув колонок в
// аркуші 1С нічого не зламає.
const ONS_COL = {CODE:1, NAME:2, PHONE:3, CITY:4, REGION:5, AGENT:6, STATUS:8, OP_STATUS:9, DATE:16};
// Заголовки статусів в аркуші 1С (пошук без урахування регістру)
const ONS_HDR_STATUS    = ["статус"];
const ONS_HDR_OP_STATUS = ["оп статус", "оп. статус", "операційний статус", "оп-статус"];
  ...
  // ── Дедуплікація ──
    ...
    if (text==="/допомога") {
      sendViber(sender.id,
        ...
        // ── v6.9 ──
        "/передати 0671234567 Дунас Богдан — передати клієнта іншому менеджеру\n"+
        "/допомога — довідка"+
    ...
    if (tl.startsWith("/1с")||tl.startsWith("/1c")) {
      ...
      if (r1c) {
        sendViber(sender.id,
          ...
          // ── v6.8: статуси клієнта в 1С ──
          "\nСтатус: "+r1c.status+"\nОп статус: "+r1c.opStatus);
      } else {
    ...
    // ── v6.9: передати контрагента іншому менеджеру ──
    // Обробник handleTransferCommand — у файлі ManagerTransfer.gs.
    // Права перевіряються всередині: адмін і керівники — будь-кого,
    // менеджер — лише своїх клієнтів.
    if (tl.startsWith("/передати")||tl.startsWith("/передать")||tl.startsWith("/transfer")) {
      handleTransferCommand(text, sender);
      return okResponse();
    }
    ...
    if (phoneMatch) {
      ...
      if (ons) {
        results.push(
          ...
          "Агент: "+ ons.agent+"\n"+
          "Дата: "+  ons.date+"\n"+
          // ── v6.8: статуси клієнта в 1С (колонки H та I) ──
          "Статус: "+    ons.status+"\n"+
          "Оп статус: "+ ons.opStatus
        );
      }
...
function parseAndSave(text,sender) {
  try {
    ...
    pushLeadToLtexCrm(row, newId, registrar); // v6.7 — + хто зареєстрував
...
function saveEmailLead(emailData, managers) {
  ...
  try {
    ...
    pushLeadToLtexCrm(row, newId, "Gmail"); // v6.7 — лід приїхав листом
...
function check1CByPhone(phone) {
  ...
  try {
    ...
    // ── v6.8: читаємо стільки колонок, скільки є в аркуші (мінімум 18),
    // щоб статуси не «випали» за межі діапазону, якщо їх посунули вправо.
    var numCols = Math.max(sheet.getLastColumn(), 18);
    var header  = sheet.getRange(1, 1, 1, numCols).getValues()[0];
    var stCol   = findOnsCol_(header, ONS_HDR_STATUS,    ONS_COL.STATUS);
    var opCol   = findOnsCol_(header, ONS_HDR_OP_STATUS, ONS_COL.OP_STATUS);
    var data    = sheet.getRange(2, 1, lastRow-1, numCols).getValues();
    // Нормалізуємо вхідний номер: залишаємо тільки останні 9 цифр
    var phone9 = phone.replace(/\D/g,"").slice(-9);
    if (phone9.length < 8) return null;
    Logger.log("check1CByPhone: шукаємо "+phone9+" серед "+data.length+" рядків (Статус=кол."+stCol+", Оп статус=кол."+opCol+")");
    for (var i=0; i<data.length; i++) {
      ...
      if (ph9 === phone9) {
        ...
        return {
          ...
          date:  dateStr,
          // ── v6.8: статуси клієнта в 1С ──
          status:   onsCell_(data[i], stCol),
          opStatus: onsCell_(data[i], opCol)
        };
      }
    }
...
// ── v6.8: пошук колонки аркуша 1С за заголовком ──────────
// Повертає номер колонки (1-based). Якщо заголовок не знайдено —
// повертає fallback (значення з ONS_COL).
function findOnsCol_(header, names, fallback) {
  for (var n = 0; n < names.length; n++) {
    var want = names[n].trim().toLowerCase();
    for (var c = 0; c < header.length; c++) {
      var h = header[c] ? header[c].toString().trim().toLowerCase() : "";
      if (h === want) return c + 1;
    }
  }
  return fallback;
}

// Значення клітинки аркуша 1С за номером колонки; порожнє → «—»
function onsCell_(row, col) {
  if (!col || col < 1 || col > row.length) return "—";
  var v = row[col-1];
...
// ── v6.8: перевірка, чи правильно визначаються колонки статусів ──
// Запустити вручну з редактора: підставити реальний номер клієнта.
function test1CStatuses() {
  var res = check1CByPhone("0662871370");
  if (!res) { Logger.log("Не знайдено в базі 1С"); return; }
  Logger.log("Код: "+res.code+"\nПІБ: "+res.name+"\nАгент: "+res.agent+
             "\nДата: "+res.date+"\nСтатус: "+res.status+"\nОп статус: "+res.opStatus);
}
...
// ── v6.7: місток «бот → L-TEX CRM» ─────────────────────────────────────────
// Надсилає щойно збережений лід у нашу систему. Best-effort: будь-яка помилка
// глушиться, робота бота й таблиці від неї не залежить.
//
// v6.7: третій аргумент addedBy — ХТО зареєстрував ліда (імʼя реєстратора з
// Viber або "Gmail" для листів). У нашій системі це видно в деталях ліда.
// Аргумент необовʼязковий: якщо не передати, все працює як у v6.6.
function pushLeadToLtexCrm(row, newId, addedBy) {
  try {
    var props = PropertiesService.getScriptProperties();
    var url = props.getProperty("LTEX_CRM_URL");
    var secret = props.getProperty("LTEX_CRM_SECRET");
    if (!url || !secret) return; // не налаштовано — тихо виходимо
    ...
    var payload = {
      ...
      category: row[COL.CAT - 1] || "",
      channel: row[COL.CHANNEL - 1] || "",
      site: row[COL.SITE - 1] || "",
      managerName: row[COL.MANAGER - 1] || "",
      addedBy: addedBy || "",
      // «Оновлений статус» (колонка R). Для щойно створеного рядка порожній,
      // але та сама функція викликається й при оновленні — тоді статус їде.
      callStatus: (COL.NEW_STATUS ? row[COL.NEW_STATUS - 1] : "") || "",
      date: row[COL.DATE - 1] || "",
    };

    // Дата може бути обʼєктом Date — приводимо до dd.MM.yyyy.
    if (payload.date instanceof Date) {
      payload.date = Utilities.formatDate(
        payload.date,
    ...
    if (!payload.name || !payload.phone) return; // немає що надсилати
    ...
    Logger.log("LTEX CRM: помилка містка — " + e);
  }
}
